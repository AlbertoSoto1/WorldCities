...
export class CountryEditComponent extends BaseFormComponent {
  ...
  country: Country;
  // the country object id, as fetched from the activate route:
  // it's NULL when we're adding a new country
  // and not NULL when we're editing and existing one.
  id?: number;
  ...
  loadData() {
    ...
    let idToInt = parseInt(this.activatedRoute.snapshot.paramMap.get('id'));
    this.id = idToInt;
